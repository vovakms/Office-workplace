        ...
        private void tabControl1_Selected(object sender, TabControlEventArgs e)//------ обработка событие выбор закладки-------------------------------------------------
        {
            ...
            if (e.TabPageIndex == 2)// клик по закладке "Заявки льготников"
            {
                ...
                if (fl2 == false)
                {
                    ...
                    conn2.Open();
                    OdbcDataAdapter da2 = new OdbcDataAdapter("fix all; select a.RQST_NUM as 'Ном', char(a.RQST_DATE) as 'Дата', b.LNAME as 'Фамилия', b.FNAME as 'Имя', b.MNAME as 'Отчество', char(a.RSLT_DATE) as 'Дата рожд.', char(b.BDATE) as 'Дата см.', b.MODPERSON as 'Отв' from LMBRQST a , LQUEUE b where  a.ID_OWN = b.ID and ( b.ID_FSS = 2711  ) ;", conn2);
                    da2.Fill(dt2);
